<script lang="ts">
  ...
  export let snapshot: SeriesSnapshot | null = null;
  ...
  export let searchMatches = 0;
  export let searchCurrentMatch = 0;
  export let searchInputActive = false;

  let searchInputEl: HTMLInputElement | null = null;

  // Auto-focus search input when active
  $: if (searchInputActive && searchInputEl) {
    tick().then(() => {
      searchInputEl?.focus();
    });
  }
  ...
  $: ageText = snapshot?.last_update_unix_ms
    ? `Upd ${Math.max(0, Math.floor((Date.now() - Number(snapshot.last_update_unix_ms as bigint)) / 1000))}s`
    : 'Upd -';
...
<section class="header" data-flag={displayFlag.toLowerCase()}>
  ...
  {#if searchInputActive || searchQuery}
    <div class="search-strip" role="search" aria-label="Search">
      ...
      />
      {#if searchMatches > 0}
        <span class="search-count">({searchCurrentMatch + 1}/{searchMatches})</span>
      {:else if searchQuery}
