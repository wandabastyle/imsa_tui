<script lang="ts">
  ...
  let rowRefs: SvelteMap<number, HTMLTableRowElement> = new SvelteMap();
  ...
  // Scroll selected row into view with enhanced behavior
  async function scrollSelectedIntoView() {
    await tick();
    if (!scrollContainer || selectedRow < 0) return;

    // Try to get the row from refs first, then fall back to querySelector
    let selectedEl: HTMLElement | null = rowRefs.get(selectedRow) ?? null;
    if (!selectedEl) {
      selectedEl = scrollContainer.querySelector('tr.selected') as HTMLElement | null;
    }
  ...
  afterUpdate(() => {
    ...
    const selected = rowRefs.get(selectedRow) ?? container.querySelector('tr.selected') as HTMLElement | null;
    if (!selected) {
    ...
    {#if isGroupedMode}
      <div class="group-stack">
        {#if groupedSections.length === 0}
          <p class="empty">{getEmptyMessage()}</p>
        {:else}
    ...
    {:else}
      <table>
        ...
        <tbody>
          {#if entries.length === 0}
            <tr>
              <td class="empty-message" colspan={activeColumns.length}>{getEmptyMessage()}</td>
            </tr>
          {:else}
            {#each entries as entry, index (entry.stable_id)}
              <tr data-index={index} data-stable-id={entry.stable_id} class={`${rowClassTint(entry) ? 'class-tint' : ''} ${rowPitPhase(entry)} ${index === selectedRow ? 'selected' : ''} ${entry.stable_id === markedStableId ? 'search-mark' : ''}`} style={rowStyle(entry, index === selectedRow)}>
                {#each cells(entry) as cell, colIndex (`${entry.stable_id}-${activeColumns[colIndex]}`)}
...
<style>
  ...
  tr.pit-active:not(.selected) {
    color: #ffd166;
    font-weight: 700;
    background: rgba(255, 221, 0, 0.15);
  }

  tr.pit-active.selected {
    color: #ffe08a;
    background: #244f82;
  }
