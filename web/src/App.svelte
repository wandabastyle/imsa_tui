<script lang="ts">
  ...
  import NlsLivetickerModal from '$lib/components/NlsLivetickerModal.svelte';
  import MessagesModal from '$lib/components/MessagesModal.svelte';
  import { fetchSessionState, loginWithAccessCode, logoutSession, updateDemoState, fetchNlsLiveticker } from '$lib/api';
  import {
    appState,
    destroyStreams,
    favouriteKey,
  ...
  onMount(async () => {
    ...
    const handler = (event: KeyboardEvent) => {
      ...
      if ($appState.showNlsLiveticker || $appState.showMessages) {
        if (event.key === 'Escape') {
          appState.update((state) => ({
            ...state,
        ...
        // Let other keys pass through for scrolling within modals
        if (event.key !== 'h' && event.key !== '?' && event.key !== 'l' && event.key !== 'm') {
          // Modal will handle its own keys
          return;
        }
      }
  ...
  {:else}
    <div class="header-row">
      <HeaderBar
        ...
        searchInputActive={$appState.search.inputActive}
      />
    ...
    <TimingTable
      ...
      series={$appState.activeSeries}
      entries={viewEntries}
      classColors={activeSnapshot?.header.class_colors as Record<string, import('$lib/types').TimingClassColor> ?? {}}
    ...
    />
    <NlsLivetickerModal
      open={$appState.showNlsLiveticker}
      entries={$appState.nlsLiveticker.entries}
      lastUpdateUnixMs={$appState.nlsLiveticker.lastUpdateUnixMs}
      lastError={$appState.nlsLiveticker.lastError}
    />
    <MessagesModal
      open={$appState.showMessages}
