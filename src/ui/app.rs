...
use crate::{
   adapters::nls::liveticker::{
      stop_liveticker_feed,
      ActiveLivetickerFeed,
      LivetickerEntry,
      LivetickerFeedKind,
...
// Switching feeds is centralized so both keyboard shortcuts and popup
// confirmation run the exact same state-reset flow as more series are added.
...
pub fn run_app(terminal: &mut Terminal<CrosstermBackend<io::Stdout>>) -> io::Result<()> {
   ...
   loop {
      ...
      // 1) pull feed updates, 2) compute derived view data, 3) render, 4) process one
      //    key event.
      if demo_mode {
         ...
         if let Event::Key(key) = event::read()? {
                  ...
                  KeyCode::Char('c') => {
                     for notice in &notices {
                        dismissed_notice_keys
                           .insert(persisted_notice_key(active_series, notice), now_unix_secs());
                     }
                     persist_dismissed_notice_keys(
                        &mut config,
                        &mut dismissed_notice_keys,
            ...
            if nls_liveticker_panel.is_open {
               match key.code {
                  ...
                  KeyCode::Up | KeyCode::Char('k') => {
                     nls_liveticker_panel.scroll = nls_liveticker_panel.scroll.saturating_sub(1);
                  },
                  KeyCode::PageDown => {
                     nls_liveticker_panel.scroll = nls_liveticker_panel
                        .scroll
                  ...
                  KeyCode::PageUp => {
                     nls_liveticker_panel.scroll = nls_liveticker_panel.scroll.saturating_sub(10);
                  },
            ...
            match key.code {
               ...
               KeyCode::Char('f') if !show_help && !current_view_entries.is_empty() => {
                  for offset in 1..=current_view_entries.len() {
                     ...
                     if favourites.contains(&fav_key) {
                        selected_row = idx;
                        gap_anchor_stable_id = Some(current_view_entries[idx].stable_id.clone());
                        break;
                     }
                  }
               },
