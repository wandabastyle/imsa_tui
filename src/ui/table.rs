...
pub(crate) fn build_table<'a>(
   ...
   let (headers, widths, imsa_widths, nls_widths, f1_widths, wec_widths): TableLayout = match ctx
      .active_series
   {
      Series::Imsa => {
...
fn build_rows(
   ...
   entries
      ...
      .map(|e| {
         ...
         let car_cell = Cell::from(format!("{fav_marker}{}", e.car_number)).style(car_cell_style);
