use ratatui::layout::Constraint;
use serde::{
   Deserialize,
   Serialize,
};

use super::width_math::{
   distribute_extra_space,
   max_text_width,
   reduce_widths_in_order,
};
use crate::timing::TimingEntry;

const NLS_COLUMN_COUNT: usize = 16;
const F1_COLUMN_COUNT: usize = 11;
const NLS_GUTTERS: u16 = 15;
const F1_GUTTERS: u16 = 10;

fn max_position_width(entries: &[TimingEntry]) -> u16 {
   let max_len = entries
      .iter()
      .map(|entry| entry.position.to_string().chars().count())
      .max()
      .unwrap_or(1);
   u16::try_from(max_len).unwrap_or(u16::MAX)
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
pub struct NlsColumnWidths {
   pos:        u16,
   car_number: u16,
   class:      u16,
   pic:        u16,
   driver:     u16,
   vehicle:    u16,
   team:       u16,
   laps:       u16,
   gap:        u16,
   last:       u16,
   best:       u16,
   s1:         u16,
   s2:         u16,
   s3:         u16,
   s4:         u16,
   s5:         u16,
}

impl NlsColumnWidths {
   const fn header_minimums() -> Self {
      Self {
         pos:        3,
         car_number: 1,
         class:      5,
         pic:        3,
         driver:     6,
         vehicle:    7,
         team:       6,
         laps:       4,
         gap:        3,
         last:       4,
         best:       4,
         s1:         2,
         s2:         2,
         s3:         2,
         s4:         2,
         s5:         2,
      }
   }

   pub fn from_entries(entries: &[TimingEntry]) -> Option<Self> {
      if entries.is_empty() {
         return None;
      }
      let pos = max_position_width(entries);
      Some(Self {
         pos,
         car_number: max_text_width(entries, |entry| &entry.car_number),
         class: max_text_width(entries, |entry| &entry.class_name),
         pic: max_text_width(entries, |entry| &entry.class_rank),
         driver: max_text_width(entries, |entry| &entry.driver),
         vehicle: max_text_width(entries, |entry| &entry.vehicle),
         team: max_text_width(entries, |entry| &entry.team),
         laps: max_text_width(entries, |entry| &entry.laps),
         gap: max_text_width(entries, |entry| &entry.gap_overall),
         last: max_text_width(entries, |entry| &entry.last_lap),
         best: max_text_width(entries, |entry| &entry.best_lap),
         s1: max_text_width(entries, |entry| &entry.sector_1),
         s2: max_text_width(entries, |entry| &entry.sector_2),
         s3: max_text_width(entries, |entry| &entry.sector_3),
         s4: max_text_width(entries, |entry| &entry.sector_4),
         s5: max_text_width(entries, |entry| &entry.sector_5),
      })
   }

   fn enforce_header_minimums(self) -> Self {
      let mins = Self::header_minimums();
      Self {
         pos:        self.pos.max(mins.pos),
         car_number: self.car_number.max(mins.car_number),
         class:      self.class.max(mins.class),
         pic:        self.pic.max(mins.pic),
         driver:     self.driver.max(mins.driver),
         vehicle:    self.vehicle.max(mins.vehicle),
         team:       self.team.max(mins.team),
         laps:       self.laps.max(mins.laps),
         gap:        self.gap.max(mins.gap),
         last:       self.last.max(mins.last),
         best:       self.best.max(mins.best),
         s1:         self.s1.max(mins.s1),
         s2:         self.s2.max(mins.s2),
         s3:         self.s3.max(mins.s3),
         s4:         self.s4.max(mins.s4),
         s5:         self.s5.max(mins.s5),
      }
   }

   const fn to_array(self) -> [u16; NLS_COLUMN_COUNT] {
      [
         self.pos,
         self.car_number,
         self.class,
         self.pic,
         self.driver,
         self.vehicle,
         self.team,
         self.laps,
         self.gap,
         self.last,
         self.best,
         self.s1,
         self.s2,
         self.s3,
         self.s4,
         self.s5,
      ]
   }

   const fn from_array(values: [u16; NLS_COLUMN_COUNT]) -> Self {
      Self {
         pos:        values[0],
         car_number: values[1],
         class:      values[2],
         pic:        values[3],
         driver:     values[4],
         vehicle:    values[5],
         team:       values[6],
         laps:       values[7],
         gap:        values[8],
         last:       values[9],
         best:       values[10],
         s1:         values[11],
         s2:         values[12],
         s3:         values[13],
         s4:         values[14],
         s5:         values[15],
      }
   }

   pub(crate) const fn driver_width(self) -> usize {
      self.driver as usize
   }

   #[must_use]
   pub(crate) const fn class_width(self) -> usize {
      self.class as usize
   }

   pub(crate) const fn vehicle_width(self) -> usize {
      self.vehicle as usize
   }

   pub(crate) const fn team_width(self) -> usize {
      self.team as usize
   }

   pub fn merge_keep_larger(self, other: Self) -> Self {
      let a = self.to_array();
      let b = other.to_array();
      let mut merged = [0_u16; NLS_COLUMN_COUNT];
      for idx in 0..NLS_COLUMN_COUNT {
         merged[idx] = a[idx].max(b[idx]);
      }
      Self::from_array(merged)
   }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
pub struct F1ColumnWidths {
   pos:        u16,
   car_number: u16,
   driver:     u16,
   team:       u16,
   laps:       u16,
   gap:        u16,
   interval:   u16,
   last:       u16,
   best:       u16,
   pit:        u16,
   stops:      u16,
}

impl F1ColumnWidths {
   const fn header_minimums() -> Self {
      Self {
         pos:        3,
         car_number: 1,
         driver:     6,
         team:       4,
         laps:       4,
         gap:        3,
         interval:   3,
         last:       4,
         best:       4,
         pit:        3,
         stops:      5,
      }
   }

   pub fn from_entries(entries: &[TimingEntry]) -> Option<Self> {
      if entries.is_empty() {
         return None;
      }
      let pos = max_position_width(entries);
      Some(Self {
         pos,
         car_number: max_text_width(entries, |entry| &entry.car_number),
         driver: max_text_width(entries, |entry| &entry.driver),
         team: max_text_width(entries, |entry| &entry.team),
         laps: max_text_width(entries, |entry| &entry.laps),
         gap: max_text_width(entries, |entry| &entry.gap_overall),
         interval: max_text_width(entries, |entry| &entry.gap_class),
         last: max_text_width(entries, |entry| &entry.last_lap),
         best: max_text_width(entries, |entry| &entry.best_lap),
         pit: max_text_width(entries, |entry| &entry.pit),
         stops: max_text_width(entries, |entry| &entry.pit_stops),
      })
   }

   fn enforce_header_minimums(self) -> Self {
      let mins = Self::header_minimums();
      Self {
         pos:        self.pos.max(mins.pos),
         car_number: self.car_number.max(mins.car_number),
         driver:     self.driver.max(mins.driver),
         team:       self.team.max(mins.team),
         laps:       self.laps.max(mins.laps),
         gap:        self.gap.max(mins.gap),
         interval:   self.interval.max(mins.interval),
         last:       self.last.max(mins.last),
         best:       self.best.max(mins.best),
         pit:        self.pit.max(mins.pit),
         stops:      self.stops.max(mins.stops),
      }
   }

   const fn to_array(self) -> [u16; F1_COLUMN_COUNT] {
      [
         self.pos,
         self.car_number,
         self.driver,
         self.team,
         self.laps,
         self.gap,
         self.interval,
         self.last,
         self.best,
         self.pit,
         self.stops,
      ]
   }

   const fn from_array(values: [u16; F1_COLUMN_COUNT]) -> Self {
      Self {
         pos:        values[0],
         car_number: values[1],
         driver:     values[2],
         team:       values[3],
         laps:       values[4],
         gap:        values[5],
         interval:   values[6],
         last:       values[7],
         best:       values[8],
         pit:        values[9],
         stops:      values[10],
      }
   }

   pub(crate) const fn driver_width(self) -> usize {
      self.driver as usize
   }

   pub(crate) const fn team_width(self) -> usize {
      self.team as usize
   }

   pub fn merge_keep_larger(self, other: Self) -> Self {
      let a = self.to_array();
      let b = other.to_array();
      let mut merged = [0_u16; F1_COLUMN_COUNT];
      for idx in 0..F1_COLUMN_COUNT {
         merged[idx] = a[idx].max(b[idx]);
      }
      Self::from_array(merged)
   }
}

pub fn calculate_nls_widths(
   terminal_width: u16,
   entries: &[TimingEntry],
   baseline: Option<&NlsColumnWidths>,
) -> NlsColumnWidths {
   let observed = NlsColumnWidths::from_entries(entries);
   let target = match (baseline.copied(), observed) {
      (Some(base), Some(obs)) => base.merge_keep_larger(obs),
      (Some(base), None) => base,
      (None, Some(obs)) => obs,
      (None, None) => NlsColumnWidths::header_minimums(),
   }
   .enforce_header_minimums();

   // Store the target class width as content minimum before any reduction
   let class_content_minimum = target.class;

   let mut widths = target.to_array();
   let header_minimums = NlsColumnWidths::header_minimums().to_array();
   let available_width = terminal_width.saturating_sub(NLS_GUTTERS);
   let total_width: u16 = widths.iter().sum();

   if total_width < available_width {
      distribute_extra_space(&mut widths, available_width - total_width);
   } else if total_width > available_width {
      let mut deficit = total_width - available_width;

      // Phase 1: Reduce non-class columns toward header minimums (protect class)
      deficit = reduce_widths_in_order(&mut widths, &header_minimums, deficit, &[6, 5]);
      deficit = reduce_widths_in_order(&mut widths, &header_minimums, deficit, &[
         1, 3, 7, 8, 9, 10, 11, 12, 13, 14, 15,
      ]);
      deficit = reduce_widths_in_order(&mut widths, &header_minimums, deficit, &[4, 0]);

      // Phase 2: If deficit remains, reduce class toward its content minimum
      if deficit > 0 {
         let mut content_minimums = header_minimums;
         content_minimums[2] = class_content_minimum;
         deficit = reduce_widths_in_order(&mut widths, &content_minimums, deficit, &[2]);
      }

      // Phase 3: Last resort - reduce all toward header minimums
      if deficit > 0 {
         deficit = reduce_widths_in_order(&mut widths, &header_minimums, deficit, &[
            6, 5, 1, 3, 7, 8, 9, 10, 11, 12, 13, 14, 15, 4, 0, 2,
         ]);
      }

      if deficit > 0 {
         widths = header_minimums;
      }
   }

   NlsColumnWidths::from_array(widths)
}

pub fn nls_constraints(widths: NlsColumnWidths) -> Vec<Constraint> {
   widths
      .to_array()
      .into_iter()
      .map(Constraint::Length)
      .collect()
}

pub fn calculate_f1_widths(
   terminal_width: u16,
   entries: &[TimingEntry],
   baseline: Option<&F1ColumnWidths>,
) -> F1ColumnWidths {
   let observed = F1ColumnWidths::from_entries(entries);
   let target = match (baseline.copied(), observed) {
      (Some(base), Some(obs)) => base.merge_keep_larger(obs),
      (Some(base), None) => base,
      (None, Some(obs)) => obs,
      (None, None) => F1ColumnWidths::header_minimums(),
   }
   .enforce_header_minimums();
   let mut widths = target.to_array();
   let minimums = F1ColumnWidths::header_minimums().to_array();
   let available_width = terminal_width.saturating_sub(F1_GUTTERS);
   let total_width: u16 = widths.iter().sum();

   if total_width < available_width {
      distribute_extra_space(&mut widths, available_width - total_width);
   } else if total_width > available_width {
      let mut deficit = total_width - available_width;
      deficit = reduce_widths_in_order(&mut widths, &minimums, deficit, &[2, 3]);
      deficit = reduce_widths_in_order(&mut widths, &minimums, deficit, &[1, 4, 5, 6, 7, 8, 9, 10]);
      deficit = reduce_widths_in_order(&mut widths, &minimums, deficit, &[0]);
      if deficit > 0 {
         widths = minimums;
      }
   }

   F1ColumnWidths::from_array(widths)
}

pub fn f1_constraints(widths: F1ColumnWidths) -> Vec<Constraint> {
   widths
      .to_array()
      .into_iter()
      .map(Constraint::Length)
      .collect()
}
