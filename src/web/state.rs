...
// latest snapshot per series, per-profile preferences, and broadcast channels
// for SSE fanout.
...
impl WebAppState {
   ...
   pub fn with_profile_cookie_secure(profile_cookie_secure: bool) -> Self {
      ...
      for series in Series::all() {
         snapshots.insert(series, SeriesSnapshot {
...
mod tests {
   ...
   fn imsa_fetching_status_appears_before_first_snapshot() {
      ...
      state.apply_timing_message(Series::Imsa, &TimingMessage::Status {
         source_id: 1,
   ...
   fn imsa_fetching_status_allowed_during_error_recovery() {
      ...
      state.apply_timing_message(Series::Imsa, &TimingMessage::Error {
         source_id: 1,
   ...
   fn other_series_status_unaffected() {
      ...
      state.apply_timing_message(Series::Nls, &TimingMessage::Status {
         source_id: 2,
      ...
      state.apply_timing_message(Series::Nls, &TimingMessage::Snapshot {
         source_id: 2,
         header:    test_header(),
         entries:   test_entries(),
      });
