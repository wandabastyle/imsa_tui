// Worker bridge: starts/stops one worker per series on demand and folds updates
// into shared web state.
...
use crate::{
   feed::{
      runtime::source_id_for,
      spawn::spawn_series_worker,
   },
   timing::{
      Series,
      TimingMessage,
   ...
   fn stop_if_still_idle(&self, series: Series, expected_generation: u64) {
      ...
      if let Some(stop_tx) = stop_tx {
         ...
         self
            .inner
            .state
            .apply_timing_message(series, &TimingMessage::Status {
