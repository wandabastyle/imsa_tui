// IMSA feed adapter: polls JSON/JSONP endpoints and normalizes rows into shared
// timing structs.
...
#[cfg(test)]
use self::parser::{
   is_transponder_placeholder,
   parse_entry,
};
use self::{
   parser::{
      fetch_snapshot,
      is_parsed_entry_transponder_placeholder,
      normalize_class_name as normalize_class_name_impl,
      FetchedSnapshot,
   },
   snapshot::{
      flush_dirty_snapshot_on_shutdown,
      imsa_snapshot_path,
...
fn handle_fetched_snapshot(
   ...
   let dirty_reason = dirty_transition_reason(previous_snapshot, &fetched.header, &fetched.entries);
